...
const NavigationLink = ({link}: {link: {name: string, sublinks: {name: string; url:string; text?: string; img?:string;  imgMobile?: string}[]}}) => {
    return(
        <li className={`nav-dropdown w-full flex items-center cursor-pointer py-12 px-4 lg:px-6 ${link.name === 'Use cases' ? 'relative' : ''}`}>
            <span className="mr-2 whitespace-nowrap">{link.name}</span>
            <span>
            ...
            {link.name !== 'Use cases' ? (
                <div className="dropdown-content w-full bg-white absolute left-0 top-20 z-10 shadow-menu-drop-down-shadow px-4">
                    {link.name === 'Business' ? (
                        <ul className="max-w-screen-lg py-20 w-full mx-auto grid grid-cols-3 gap-4 lg:gap-6">
                            {link.sublinks.map((sublink, i) => {
                                return(
                                    <li>
                                        <Link to={sublink.url} className="flex space-x-5 items-start">
                                            <img src={sublink.img} alt="" />
                        ...
                        <ul className="max-w-screen-lg py-10 w-full mx-auto grid grid-cols-3 gap-12">
                            {link.sublinks.map((sublink, i) => {
                                return(
                                    <li className="">
                                        <Link to={sublink.url}>
                 ...
                 <div className="dropdown-content bg-white absolute py-10 left-0 top-20 z-10 shadow-box-shadow w-64 rounded">
                     <ul>
                         {link.sublinks.map((sublink, i) => {
                             return(
                                 <li className="py-3 text-grey capitalize hover:bg-gray-50 px-4 text-sm">
